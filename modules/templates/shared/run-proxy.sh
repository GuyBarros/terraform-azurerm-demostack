...
echo "--> creating run-proxy.sh for consul connect demo"
sudo tee /usr/local/bin/run-proxy.sh > /dev/null <<"EOF"
#!/bin/bash
...
echo "--> creating run-proxy.sh for consul connect demo done"